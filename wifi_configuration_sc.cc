...
void WifiConfigurationSc::IpEventHandler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
  ...
  case IP_EVENT_STA_GOT_IP:
  {
    ...
    */
    ...
    ESP_LOGW(TAG, "Preparing to bind UID: %s", uid.c_str());
    ...
    cJSON_AddRawToObject(json_root, "userId", uid.c_str());
    ...
    ESP_LOGW(TAG, "JSON Data: %s", json_data);
    int json_data_len = strlen(json_data);
    cJSON_Delete(json_root);

    //*初始化http client
    char *local_response_buffer = (char *)heap_caps_calloc(1024, 1, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
    assert(local_response_buffer != nullptr);
    esp_http_client_config_t http_client_config = {
        .url = "",                                  // 完整的URL地址（包含协议、主机、路径）
        ...
        .buffer_size = 1024,                       // HTTP接收缓冲区大小
        .buffer_size_tx = 1024,                    // HTTP发送缓冲区大小
        .user_data = local_response_buffer,        // 用户自定义数据指针
...
#endif
        .addr_type = HTTP_ADDR_TYPE_INET, // 地址类型（未指定/IPv4/IPv6）
#if CONFIG_MBEDTLS_DYNAMIC_BUFFER
        .tls_dyn_buf_strategy = (esp_http_client_tls_dyn_buf_strategy_t)0, // TLS动态缓冲区策略
#endif
    };
    //TODO: 配网完成后测试网络, 并绑定设备
    //*客户端初始化 
    self->http_client_ = esp_http_client_init(&http_client_config);
    esp_http_client_handle_t client = self->http_client_;
    //*设置GET请求
    char *url_cstr = (char *)heap_caps_calloc(512, 1, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM); //!
    sprintf(url_cstr, "http://xjai.art:8002/xiaozhi/xuanji/getBindedDevices/%s", self->uid_.c_str());
    ESP_ERROR_CHECK(esp_http_client_set_url(client, url_cstr));
    ...
    //* 设置POST请求
    memset(url_cstr, 0, 512); //!
    sprintf(url_cstr, "http://xjai.art:8002/xiaozhi/xuanji/newset");
    ESP_ERROR_CHECK(esp_http_client_set_method(client, HTTP_METHOD_POST));
    ESP_ERROR_CHECK(esp_http_client_set_header(client, "Content-Type", "application/json"));
    ESP_ERROR_CHECK(esp_http_client_set_post_field(client, json_data, json_data_len));

    //* 执行POST请求 esp_http_client_perform会阻塞直到请求完成
    ...
    heap_caps_free(url_cstr); //!------------------------------------
    cJSON_free(json_data);    //!------------------------------------



    UidManager::GetInstance().SetUid(uid); // 保存UID
    xEventGroupSetBits(self->event_group_, WIFI_CONNECTED_BIT);

    //*GUI更新
    auto &application = Application::GetInstance();
    application.Alert(Lang::Strings::WIFI_CONFIG_MODE, "配网完成,即将重启", "gear", "");
    vTaskDelay(pdMS_TO_TICKS(1000));
    //重启设备
    for (int i = 0; i < 3; i++)
    {
      char buf[64] = {0};
      snprintf(buf, sizeof(buf), " %d 秒后重启...", 3 - i);
      application.Alert(Lang::Strings::WIFI_CONFIG_MODE, buf, "gear", "");
      vTaskDelay(pdMS_TO_TICKS(1000));
    }
    esp_restart();
...
void WifiConfigurationSc::SmartConfigEventHandler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
  ...
  case SC_EVENT_GOT_SSID_PSWD:
  {
    ...
    if (at_pos != std::string::npos)
    {
      ...
      ESP_LOGI(TAG, "class UID: %s", self->uid_.c_str());
    }
    else
    {
      std::lock_guard<std::mutex> lock(self->mutex_);
